...
def sugestao(request):
    ...
    if request.method == 'GET':
        ...
        if tipo == 'C':
            empresas = Empresas.objects.filter(tempo_existencia='+5')\
                                        .filter(estagio='E')
        elif tipo == 'D':
            empresas = Empresas.objects.\
            filter(tempo_existencia__in=['-6', '+6', '+1']).exclude(estagio='E')
        #To Do: tipo genérico
...
def ver_empresa(request, id):
    ...
    #To Do: listar as métricas dinamicamente
...
def realizar_proposta(request, id):
    ...
    propostas_aceitas = PropostaInvestimento.objects.filter(empresa=empresa)\
                        .filter(status='PA')
    total = 0
    ...
    if total + int(percentual) > empresa.percentual_equity:
        messages.add_message(request, constants.WARNING, 
                       'O percentual solicitado ultrapassa o percentual maximo')
    ...
    pi = PropostaInvestimento(valor = valor, percentual=percentual, 
                              empresa=empresa, investidor=request.user)
...
def assinar_contrato(request, id):
    ...
    if request.method == 'GET':
        ...
        pi.status = 'PE'
        pi.save()
        messages.add_message(request, constants.SUCCESS, f'Contrato assinado')
